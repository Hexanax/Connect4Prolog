...
END_AI_ID = 13 # le max c'est 8, mais il ya un bug dans l'un d'entre eux, a vous de trouver lequel...

bar = Bar('Processing', max=(END_AI_ID-START_IA_ID) ** 2, suffix = '%(percent).1f%% - %(eta)ds')
for i in range(START_IA_ID,END_AI_ID + 1):
	lines[0] = lines[0]+str(i)+';'
	lines[i-1] = str(i)+';'

for i in range(START_IA_ID,END_AI_ID + 1):
	for j in range(i,END_AI_ID + 1): 
		bar.next()
